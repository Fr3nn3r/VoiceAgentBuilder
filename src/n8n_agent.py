...
from livekit.agents.llm import LLM, LLMStream, ChatContext, ChatChunk, ChoiceDelta, ChatMessage
...
class N8nWebhookLLM(LLM):
    ...
    def chat(
        ...
        *,
        ...
        logger.debug(f"[N8N] ChatContext attributes: {[attr for attr in dir(chat_ctx) if not attr.startswith('_')]}")
        ...
        try:
            ...
            for msg in reversed(messages):
                logger.debug(f"[N8N] Message: role={getattr(msg, 'role', 'unknown')}, content type={type(getattr(msg, 'content', None))}")
                ...
                if hasattr(msg, 'role') and msg.role == "user":
                    ...
                    if user_message:
                        logger.info(f"[N8N] Successfully extracted user message: {user_message}")
                        break
...
class N8nLLMStream(LLMStream):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.debug(f"[N8N Stream] Exiting async context manager (exc_type: {exc_type})")
    ...
    async def _fetch_response(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    logger.info(f"[N8N Stream] Received response: status={response.status}")
                    if response.status == 200:
                        ...
                        if isinstance(data, dict):
                            ...
                            text = (data.get("output") or
                                   ...
                                   data.get("message") or
                                   "")
                            # If text is still a dict, convert to string
                        ...
                        logger.error(f"[N8N Stream] Webhook returned status {response.status}, body: {body}")
                        text = "I'm having trouble processing your request right now."

        except aiohttp.ClientTimeout:
            logger.error(f"[N8N Stream] Timeout after {self.timeout} seconds")
        ...
        self._llm._cursors[self._chat_ctx] = ChatMessage(role="assistant", content=[text])

        # Emit response in chunks for smoother TTS
        chunk_size = 50
        num_chunks = (len(text) + chunk_size - 1) // chunk_size
        logger.debug(f"[N8N Stream] Chunking response into {num_chunks} chunks of {chunk_size} chars")
        ...
        for i in range(0, len(text), chunk_size):
            ...
            logger.debug(f"[N8N Stream] Sending chunk {i//chunk_size + 1}/{num_chunks}: {chunk_text[:20]}...")
    ...
    async def __anext__(self):
        ...
        try:
            ...
            logger.debug(f"[N8N Stream] Received chunk: {chunk.delta.content if chunk.delta else 'empty'}")
            return chunk
